...
class RetailBillingSystem:
        ...
        try:
            ...
            for i, (item, price, category, stock) in enumerate(self.items):
                ...
                button = self.create_button(
                    ...
                    text=f"{item}\n${price:.2f}",
                    row=i // 5, column=i % 5,
                    command=lambda item=item, price=price: self.add_text(item, price)
                )
    ...
    def update_receipt_listbox(self):
        ...
        self.receipt_listbox.insert(tk.END, "-" * 50)
        self.receipt_listbox.insert(tk.END, "{:<20}:{:<20}".format(translate("Table"), self.table.replace("table","")))
...
class TableMapApp:
    ...
    def create_menu(self):
        ...
        self.context_menu.add_command(label="Edit Status", command=self.edit_vieww)
    ...
    def sub_view_item(self):
        ...
        table_number = self.tree.item(selected_item, "values")[0]
        self.details_window = tk.Toplevel()
        details_window = self.details_window
        details_window.title("Table Order Details")
        ...
        for order in self.tables[table_number]["order"]:
            tree.insert("", "end", values=(order['Item'], order['Price'], order['Qty']))
    ...
    def create_table_editor(self):
        ...
        self.tree.bind("<Double-1>", self.select_and_open_system)
    ...
    def edit_vieww(self, event = None):
        ...
        selected_item = self.tree.selection()[0]
        table_number = self.tree.item(selected_item, "values")[0]
        if self.tables[table_number]['order']:
            ttk.Label(edit_window, text="Unavailable").grid(row=1, column=1, padx=10, pady=10)
        else:
    ...
    def save_changes(self, item, table, status, remark):
        ...
        selected_item = self.tree.selection()[0]
        table_number = self.tree.item(selected_item, "values")[0]
        if status == "Unavailable" and not self.tables[table_number]['order']:
    ...
    def update_status_table(self):
        for table_id, table_info in self.tables.items():
            if table_info['order'] or table_info['status'] == 'Fully unavailable':
                table_info["status"] = "Unavailable"
            elif table_info['status'] == 'Reservation' and not table_info['order']:
                continue
            elif table_info['status'] == 'Occupied' and not table_info['order']:
                continue
            elif not table_info['order'] or table_info['status'] == 'Available':
    ...
    def select_and_open_system(self, event=None):
        try:
            selected_item = self.tree.selection()[0]
            table_id = self.tree.item(selected_item, "values")[0]
            self.open_order_system(table_id)
        except IndexError:
    ...
    def get_table_color(self, status):
        return {"Available": "green", "Unavailable": "dark slate gray", "Fully unavailable": "dark slate gray", "Reservation": "orange", "Occupied": "gray"}.get(status, "gray")
